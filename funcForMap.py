        ...
        for j in range(len(map[i]) - 1):
            if map[i][j] == map[i][j + 1] == map[i + 1][j] == map[i + 1][j + 1] and int(map[i][j]) != 0:
                temp = polygon_center([((len(map[i]) - j - 1) * c, c * int(map[i][j]), c * i), ((len(map[i]) - j) * c, c * int(map[i][j + 1]), c * i),
                                ((len(map[i + 1]) - j) * c, c * int(map[i + 1][j]), (i + 1) * c),
                ...
                if map[i][j] == map[i + 1][j]:
                    temp = polygon_center([((len(map[i]) - j - 1) * c, c * int(map[i][j]), i), ((len(map[i + 1]) - j) * c, c * int(map[i][j]), c * i),
                                     ((len(map[i + 1]) - j) * c, 0, i * c), ((len(map[i]) - j - 1) * c, 0, c * i)])
